...
erDiagram
    ...
    SYLLABUS_CLO {
        int id PK "Khóa chính tự tăng"
        int syllabus_id FK "Liên kết Syllabus ID [Xóa Cascade]"
        varchar clo_name "Ký hiệu chuẩn đầu ra (Ví dụ: CLO1)"
        text clo_details "Nội dung chuẩn đầu ra chi tiết"
        text lo_details "Liên kết chuẩn đầu ra chương trình (LO)"
    }
...
CREATE TABLE syllabuses (
    id INT PRIMARY KEY, -- Sử dụng ID Syllabus của FLM làm Khóa chính
    ...
    approved_date DATE,
    min_avg_mark_to_pass NUMERIC(4, 2) DEFAULT 5.0,
...
CREATE TABLE syllabus_clos (
    id SERIAL PRIMARY KEY,
    syllabus_id INT NOT NULL REFERENCES syllabuses(id) ON DELETE CASCADE,
    clo_name VARCHAR(50) NOT NULL, -- Ví dụ: 'CLO1', 'CLO2'
    clo_details TEXT NOT NULL,
    lo_details TEXT
);
...
CREATE TABLE assessment_schemes (
    ...
    part VARCHAR(50),
    weight NUMERIC(5, 2) NOT NULL,  -- Trọng số phần trăm thực tế (Ví dụ: 15.00 = 15%)
